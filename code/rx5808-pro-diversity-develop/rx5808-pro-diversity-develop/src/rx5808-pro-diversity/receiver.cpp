...
namespace Receiver {
    ...
    static Timer rssiLogTimer = Timer(RECEIVER_LAST_DELAY);
        ...
        static Timer serialLogTimer = Timer(500);
    #endif
    ...
    void update() {
        if (rssiStableTimer.hasTicked()) {
            ...
            #if defined(USE_DIVERSITY) && !defined(RSSI_SERIAL_DIAGNOSTIC)
                switchDiversity();
            #endif
        }
    }
...
static void writeSerialData() {
    if (Receiver::serialLogTimer.hasTicked()) {
        ...
        Serial.print(PSTR2("\t"));
        Serial.print(Channels::getFrequency(Receiver::activeChannel), DEC);
        Serial.print(PSTR2("\t"));
        Serial.print(Receiver::rssiA, DEC);
        Serial.print(PSTR2("\t"));
        Serial.println(Receiver::rssiARaw, DEC);
